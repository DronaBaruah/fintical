                ...
                <div class="md:flex w-full m-auto">
                    ...
                    <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                        <div class="text-center mb-10                        ">
                            <h1 class="font-bold text-xl text-gray-900 uppercase">Loan Interest</h1>
                            <!-- <p>Enter your information to register</p> -->
                        </div>

                        @if (session()->has('message'))
                        <div class="mb-10 bg-blue-100 border border-green-400 text-gray-800 px-4 py-3 rounded-md relative interestModal"
                            id="interestModal" role="alert">
                            ...
                            <div class="text-center">{{ session()->get('transaction_id') }}
                                <span class="absolute top-0 bottom-0 right-0 md:px-4 pr-2 py-3">
                                    <svg class="fill-current h-6 w-6 text-red-500 closeModal" role="button"
                                        id="closeModal" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                        <title>Close</title>
                                        <path
                        ...
                        <form method="GET" action="/interest/create">
                            @csrf
                            <div>
                                <div class="flex -mx-3">
                                    <div class="w-full px-3 mb-12">
                                        <label for="" class="text-xs font-semibold px-1">Select Member</label>
                                        <div class="flex">
                                            <select placeholder="Select Member" class="w-full" name="userid" id="select"
                                                required autofocus>
                                                <option value="" selected>Select Member</option>

                                                @foreach($members as $member)
                                                <?php $loan_due = 0; ?>
                                                <?php $loan = $member->loan->sum('amount'); ?>
                                                <?php $disburse = $member->disburse->sum('amount'); ?>
                                                <?php $loan_due = $loan - $disburse; ?>

                                                @if($loan_due != "0")
                                                <option value="{{ $member->id }}">{{ $member->name }}
                                                    ({{ $member->phone_no }})</option>
                                                @endif
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <div class="flex -mx-3 md:pb-0 pb-28">
                                    <div class="w-full px-3 mb-5">
                                        <button type="submit"
                                            class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">Next</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                            ...
                            <table class="min-w-max w-full table-auto">
                                <thead>
                                    <tr class="bg-gray-200 text-gray-800 uppercase text-xs leading-normal">
                                        ...
                                        <th class="py-2 px-4 text-center">Total Interest</th>
                                        <th class="py-2 px-4 text-center">Current Interest</th>
                                        <th class="py-2 px-4 text-center">Previous Interest</th>
                                        <th class="py-2 px-4 text-center">Late Fine Interest</th>
                                ...
                                <tbody class="text-gray-700 text-sm bg-white ">
                                    ...
                                    <tr class="border-b border-gray-200 hover:bg-gray-100 text-sm">
                                        ...
                                        <td class="py-2 px-4 text-center whitespace-nowrap">
                                            <div class="flex items-center justify-center">
                                                <span>{{ $interest->interest_id }}</span>
                                            </div>
                                        </td>
                                        <td class="py-2 px-4 text-center whitespace-nowrap">
                                            <div class="flex items-center justify-center">
                                                <span>{{ $interest->user->name }}</span>
                                            </div>
                                        </td>

                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                <span>{{ date('jS M Y', strtotime($interest->date)) }}</span>
                                            </div>
                                        </td>

                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                <span>₹ {{ $interest->total_interest }}</span>
                                            </div>
                                        </td>
                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                        ...
                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                <span>₹ {{ $interest->previous_interest }}</span>
                                            </div>
                                        </td>
                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                <span>₹ {{ $interest->lif_amount }}</span>
                                            </div>
                                        </td>
                                        <td class="py-2 px-4 text-center">
                                            <div class="flex items-center justify-center">
                                                <span>{{ $interest->remark  }}</span>
                                            </div>
                                        </td>
